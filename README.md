# housing-market-trends-tableau-analysis
Visualization of housing market trends and analysis of sales prices and property features using Tableau dashboards and data analytics techniques.
# Visualization of Housing Market Trends and Analysis of Sales Prices and Features using Tableau
---
##  📌 Project Dashboard URL  
To view the Dashboard clik link below.
https://public.tableau.com/views/Visualzationhousingmarkettrends/Sheet1?:language=en-US&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link
---
## Project Overview
This project focuses on analyzing housing market trends by studying property sales prices and important housing features using Tableau. The main objective is to transform raw housing data into meaningful visual insights that help users understand price variations, market behavior, and factors influencing property values.

The project applies data analytics concepts such as data collection, data cleaning, visualization, dashboard creation, and storytelling to support better decision-making in the real estate domain.
---

## 🎯 Problem Statement
Housing prices vary based on multiple factors such as location, size, number of rooms, and property features. However, raw datasets are difficult to understand without proper analysis and visualization. This project solves the problem by creating interactive dashboards that clearly represent housing market trends and price patterns.

---

## 💡 Proposed Solution
The solution uses Tableau to design interactive dashboards and stories that:
- Analyze housing price distribution
- Identify key features affecting house prices
- Compare market trends across different conditions
- Provide visual insights for buyers, sellers, and analysts

---

## 🛠️ Technology Stack
- Tableau (Data Visualization & Dashboard)
- Python (Data preparation support)
- Flask (Web application execution)
- HTML, CSS (Web interface)
- MS Excel / Dataset Files
- <img width="782" height="632" alt="image" src="https://github.com/user-attachments/assets/a1abb8bf-2992-4600-982b-f2a2b7051dc2" />


---

## 📊 Project Features
- Interactive Tableau Dashboards
- Data Storytelling Visualization
- Housing Market Trend Analysis
- Feature-based Price Comparison
- Clean and Structured Data Reports

---
 
 


